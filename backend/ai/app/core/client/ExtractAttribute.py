...
step1 - When <color> exists, ignore the color guessed from the photo and apply the <color> value first.
    ...
    def make_user_content(self) -> list:
        ...
        for data in self.dataList:
            content_list.extend([{
                ...
                "type": "text",
                "text": f"<clothes><order>{data.clothId}</order><type>{data.category}</type>{('<color>' + data.color+'</color>') if data.color else ''}</clothes>"
            }])
        return content_list
...
class ExtractAttributesImage(ExtractAttribute):
    ...
    def __init__(self, image: UploadFile, type:str=""):
        self.image = image
        self.type=type
    def make_user_content(self) -> list:
        return [{
            ...
            "type": "text",
            "text": f"<clothes><order>{0}</order><type>{type}</type></clothes>"
        }]
